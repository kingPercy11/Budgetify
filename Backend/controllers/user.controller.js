...
module.exports.updateUsername = async (req, res, next) => {
    ...
    try {
        ...
        const existingUser = await userModel.findOne({ username: newUsername, _id: { $ne: userId } });
        if (existingUser) {
        ...
        // Update all expenditures with the old username to use the new username
        await expenditureModel.updateMany(
            { username: oldUsername },
            { username: newUsername }
        );

        // Update limits with the old username to use the new username
